...
export async function GET(context: APIContext) {
  const entries = await getCollection("hooks", ({ data }) => !data.draft);
  ...
  const items = sorted.slice(0, 50).map((entry) => ({
    ...
    link: urlForEntry("hooks", entry),
    pubDate: entry.data?.updatedAt ? new Date(entry.data.updatedAt) : new Date(0),
  }));

  return rss({
    title: "Claudient — Hooks",
    description: "New and updated Claude Code hook automations from the Claudient knowledge base.",
