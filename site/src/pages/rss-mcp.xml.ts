...
export async function GET(context: APIContext) {
  const entries = await getCollection("mcp", ({ data }) => !data.draft);
  ...
  const items = sorted.slice(0, 50).map((entry) => ({
    ...
    link: urlForEntry("mcp", entry),
    pubDate: entry.data?.updatedAt ? new Date(entry.data.updatedAt) : new Date(0),
  }));

  return rss({
    title: "Claudient — MCP Servers",
    description: "New and updated MCP server configurations from the Claudient knowledge base.",
