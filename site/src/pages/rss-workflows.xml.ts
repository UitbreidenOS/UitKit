...
export async function GET(context: APIContext) {
  ...
  const items = sorted.slice(0, 50).map((entry) => ({
    ...
    link: urlForEntry("workflows", entry),
    pubDate: entry.data?.updatedAt ? new Date(entry.data.updatedAt) : new Date(0),
  }));

  return rss({
    title: "Claudient — Workflows",
    description: "New and updated Claude Code workflow guides from the Claudient knowledge base.",
