...
Initialiseert multi-agent team door rollen te definiëren, taak omvang in te delen en coördinatie contracten tussen agenten in te stellen voor executie.

---

## Wanneer gebruiken

- Starten van nieuwe multi-agent workflow met 3+ agenten
- Onboarding van nieuwe agenten in bestaand team
- Schalen van workflow om grotere, complexere taken aan te kunnen
- Zeker stellen van duidelijke rol grenzen om agent conflicten te voorkomen
- Communicatie protocollen vestigen voor agents worden losgelaten

Niet gebruiken voor single-agent taken of workflows waar agent rollen al duidelijk gedefinieerd en in productie zijn.

---

## Rol toewijzing

Elke agent ontvangt rol kaart die definieert wat deze bezit, van wat deze afhangt en wat deze niet mag doen:
...
**Rol kaart regels:**
- Één rol per agent (agenten hebben geen meerdere rollen)
- Grenzen zijn strikt — agenten moeten werk buiten hun domein afwijzen
- Afhankelijkheden definiëren sequencering en blokkerings relaties
- SLA definieert contract voor aanvaardbare performance

---

## Omvang partitionering

Verdeel algemene taak in niet-overlappende subtaken toegewezen aan specifieke agenten:
...
**Partitionerings regels:**
- Geen taak overlap (elke taak aan exact één agent toegewezen)
- Geen gaten (unie van alle taken = algemeen doel)
- Expliciete afhankelijkheden (welke subtaak moet voor ander gereedkomen)
- Duidelijke acceptatie criteria (hoe weet volgende agent dat werk gedaan is?)

---

## Coördinatie contract

Voor executie, alle agenten accepteren coördinatie contract:
...
**Contract handhaving:**
- Alle agenten verifiëren contract voorwaarden bestaan voor start
- Schendingen (bijv. circulaire call) triggeren onmiddellijke escalatie
- Supervisor handhaaft timeout, retry en resource limites
- Audit log registreert alle contract checks

---

## Pre-executie checklist

Voor release van agenten:
...
Ga alleen verder met executie als alle checklist items waar zijn.
